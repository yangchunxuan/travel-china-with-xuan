...
export async function generateMetadata({
  ...
  return getSearchHubMetadata("when-to-go", locale);
}

export default async function LocalizedWhenToGoHubPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale: routeLocale } = await params;
  const locale = localizedRouteLocale(routeLocale);
  return <SearchPlatformHubPage locale={locale} section="when-to-go" />;
}
