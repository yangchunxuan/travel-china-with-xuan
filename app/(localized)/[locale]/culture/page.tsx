...
export async function generateMetadata({
  ...
  return getSearchHubMetadata("culture", locale);
}

export default async function LocalizedCultureHubPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale: routeLocale } = await params;
  const locale = localizedRouteLocale(routeLocale);
  return <SearchPlatformHubPage locale={locale} section="culture" />;
}
