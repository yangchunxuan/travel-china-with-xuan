...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "Questions families ask before booking", items: [
    { question: "Does my child need their own train ticket in China?", answer: "Current 12306 guidance says children aged 6 to under 14 should use a child discount ticket, while each ticketed adult may carry one child under 6 free if that child does not occupy a separate seat. Age is calculated on the travel date, a free child must be declared using a valid identity document, and a child who needs a separate seat requires the applicable ticket. Recheck the rule before purchase." },
    { question: "How many cities should we plan in ten days with a toddler?", answer: "One hotel change is usually enough. A ten-day example uses Beijing for nights 1-5 and Shanghai for nights 6-9: nine nights and one move. Compare arriving in Beijing and flying home from Shanghai. This is a pacing example, not a promise of stroller access or a fixed package." },
    { question: "The train is short — should we add another city for one attraction?", answer: "Not if that is the only argument for it. The real move requires checkout, a large station, suitcases, a stroller and a room that may not be ready, so a city offering one activity can lose more usable time than it gains. A focused day visit from a stable base, or removing the stop, may protect the activity without adding a full transfer day." },
    { question: "How much can we plan in one day with young children?", answer: "One protected anchor per day. Place optional activities nearby so dropping them does not create a cross-city failure, and reserve a recovery block after every intercity move instead of attaching a timed highlight to the scheduled arrival. When the route is too full, delete a whole base before compressing sleep, meals and transfer margin." },
    { question: "What do we do if our child sleeps through a timed booking?", answer: "Let one adult attend only if the family agreed beforehand; otherwise release the booking. The design change is to keep irreplaceable anchors out of the usual nap or arrival window. Predictable food belongs before the anchor rather than after it, and a familiar backup is worth carrying in case of a refusal before a long queue." },
    { question: "Can we count on using a stroller in Chinese stations?", answer: "Check each one rather than assuming. Confirm the exact station entrances, lifts or accessibility assistance with the operator when they matter, and keep a carrying or alternate-transport fallback. Station facilities and temporary access routes can change, and one station that worked tells you nothing about stroller access nationwide." },
  ] },
