...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "별도 발권 자주 묻는 질문", items: [
    { question: "한 번에 예약했으면 연결이 보호되나요?", answer: "그것만으로는 아닙니다. 중국민용항공국 안내는 같은 항공권 번호에 있는 두 편 이상의 항공편을 연결 여정으로 설명하고, 여러 항공편을 예약할 때 판매자가 연결편 여부를 알려야 한다고 합니다. 비연결 항공편은 보통 별도 계약으로 독립 발권되어 한 편의 문제가 다른 표를 자동으로 바꾸지 않고, 각 표는 해당 운항사 조건을 따릅니다. 결제 화면 하나나 대행사 주문 번호만으로는 충분하지 않습니다." },
    { question: "위탁 수하물이 최종 목적지까지 가나요?", answer: "판매자가 서면으로 그렇게 밝힌 경우에만 해당합니다. 위탁 수하물이 최종 목적지까지 연결되는지 직접 묻고 서면 상품 설명을 저장하세요. 동맹, 공동 브랜드나 공항 구조로 추정하면 안 됩니다. 답이 조건부이거나 불명확하면 별도 계약으로 가격을 계산하고 짐을 찾아 다시 부쳐야 한다고 가정하세요. 당일 간격이 빠듯하면 짐 찾기와 재위탁에 단단한 마감이 생깁니다." },
    { question: "별도 발권 항공편 사이에 시간을 얼마나 둬야 하나요?", answer: "보편적인 숫자는 없습니다. 항공기에서 내리기, 수속, 짐, 이동, 체크인, 보안과 게이트 접근 등 모든 필수 시간을 먼저 넣고, 현재 공항·운항사 정보를 사용해 노선과 계절에 맞는 운영 불확실성을 더하세요. 다음 가능한 항공편이 드물면 복구 시간도 더합니다. 명목상 간격이 길어도 뒤에 선택지가 없으면 약합니다." },
    { question: "예약 사이트가 내세우는 셀프 환승 보장은 믿어도 되나요?", answer: "행동하기 전에 그 보장의 정확한 자격, 통지와 증빙 요건을 읽으세요. 대행사가 홍보하는 셀프 환승 보장은 판매자 자신의 상품이므로, 문제가 생겼을 때 나에게 무엇을 요구하는지 확인하고 운항사 메시지, 영수증과 원래 상품 설명을 저장해 두세요. 보장 여부는 각 약관이 정합니다." },
    { question: "첫 항공편이 지연돼 다음 편을 놓칠 것 같습니다.", answer: "먼저 첫 운항사에서 상태를 확인하고, 두 번째 체크인 마감 전에 그 운항사나 판매자에게 연락해 별도 발권임을 설명한 뒤 유료 또는 재량 선택을 물으세요. 자동 보호를 주장하면 안 됩니다. 그런 다음 마지막 당일 대안과 1박 후 다음 날 출발을 비교하고 운항사 메시지와 영수증을 저장하세요." },
    { question: "귀국 국제선 전에 당일 셀프 환승을 넣어도 되나요?", answer: "권하지 않습니다. 다음이 국제선 출국이면 국내선 한 번의 이상이 가장 비싼 표를 잃게 할 수 있으므로, 더 나은 선택은 전날 출국 도시에 도착하는 것입니다. 실패가 최종 국제선 출국을 위협한다면 몇 분을 다투지 말고 첫 항공편을 전날로 옮기세요." },
  ] },
