...
const body = {
  ...
  blocks: [
    ...
    { id: "quick-answer", type: "callout", title: "정답을 정하기 전에 전체 동선을 비교한다", tone: "decision", body: "광저우남역과 홍콩서카오룽이 모두 동선에 맞으면 직통 고속철도를 먼저 확인한다. 같은 날짜의 광저우동역 직통편도 별도로 검색한다. 12306은 현재 국경 간 상품에서 광저우남역과 광저우동역을 구분하지만 모든 날짜에 모든 역 조합의 좌석이 있는 것은 아니다. 도로 이동은 정식 운행 버스의 정확한 승하차 지점이 환승을 충분히 줄일 때만 최종 후보로 둔다." },
    ...
    { id: "decision-matrix", type: "table", caption: "짧은 승차 시간이 짧은 이동일을 뜻하지는 않는다", columns: ["선택", "잘 맞는 경우", "놓치기 쉬운 부담", "구매 전 확인"], rows: [
      ...
      ["광저우동역 직통 열차", "해당 날짜에 운행하며 시내 횡단을 줄일 때", "날짜별로 다른 좌석과 맞지 않는 시간대", "날짜가 적힌 열차와 표의 전체 역명"],
      ["국경 간 직통버스", "날짜와 운행 자격이 확인되고 승하차 지점이 유용할 때", "도로 정체, 출입경 절차, 짐과 운영사별 정류장", "운영사, 출입경 지점, 양쪽 정류장과 변경 규정"],
    ...
    { id: "time-heading", type: "heading", level: 2, text: "출발지부터 최종 목적지까지 한 번에 계산한다" },
    { id: "time-steps", type: "list", ordered: true, items: ["숙소에서 올바른 역 입구나 버스 정류장까지.", "역 내부 도보, 보안검색과 운영사가 요구하는 여유 시간.", "열차 또는 도로 이동 시간만 전체로 보지 않기.", "지정 역이나 출입경 시설의 수속 절차.", "도착 홀에서 홍콩 최종 주소까지.", "다음 고정 예약을 지킬 지연 여유."] },
    ...
    { id: "people-comparison", type: "comparison", title: "짧은 승차보다 편의가 중요한 때", columns: [
      ...
      { heading: "큰 짐 또는 고정 일정", items: ["역 통로와 출입경 이동까지 계산", "체크인이나 항공편을 놓치지 않을 여유 확보", "작은 절약보다 대체 경로가 있는 선택 우선"] },
    ]},
    ...
    { id: "late-table", type: "table", caption: "이동 다음의 이동을 확인한다", columns: ["위험", "먼저 물을 것", "안전한 대응"], rows: [
      ["서카오룽 늦은 도착", "출입경 뒤 마지막 도시 교통이 남아 있는가", "출발 전 공식 택시 승차장과 예상 비용 확인"],
      ["버스 지연", "후속 예약을 놓치지 않을 시간 여유가 있는가", "항공편이나 환불 불가 행사와 가깝게 연결하지 않기"],
      ["잘못된 광저우역", "출발 전 공식 변경이 가능한가", "잘못된 역으로 계속 가지 말고 공식 채널 사용"],
      ["직통 매진", "다른 출발역이 출발지부터 목적지까지의 이동을 줄이는가", "늦은 직통과 선전 경유 이동의 모든 위험 비교"],
    ]},
    { id: "recovery-heading", type: "heading", level: 2, text: "계획이 끊어졌을 때" },
    { id: "recovery-list", type: "list", ordered: true, items: ["기억에 의존하지 말고 예약에서 전체 역명과 정류장명을 읽는다.", "항공편, 체크인 마감, 행사 중 놓쳤을 때 손실이 가장 큰 일정부터 지킨다.", "운영사 공식 채널로 좌석과 변경을 확인한다.", "선전을 거치면 한 출입경 지점을 정하고 양쪽 연결을 확인한다.", "늦을 가능성이 생기면 숙소에 알린다."] },
    { id: "fact-check", type: "callout", title: "동적 교통 정보 확인일: 2026년 8월 13일", tone: "neutral", body: "MTR 고속철도 검색은 현재 홍콩서카오룽을 홍콩 쪽 끝점으로 두고 날짜와 중국 본토 역별로 조회한다. 직통편, 요금, 정차역, 버스 정류장, 국경 운영 시간과 입국 조건은 바뀔 수 있다. 이 글은 고정 시간표나 영구 요금을 제시하지 않는다." },
    ...
    { id: "faq", type: "faq", title: "광저우–홍콩 자주 묻는 질문", items: [
      { question: "광저우에서 홍콩까지 가장 좋은 방법은 무엇인가요?", answer: "광저우남역과 홍콩서카오룽이 일정에 맞으면 직통 고속철도부터 확인하고, 정확한 날짜로 광저우동역도 따로 검색하세요. 12306의 국경 통과 상품은 두 역을 모두 인식하지만 모든 역 조합에 매일 좌석이 있지는 않습니다. 허가받은 국경 통과 버스는 확인된 승하차 지점이 환승을 충분히 줄여 도로와 국경의 불확실성을 상쇄할 때만 비교하세요." },
      { question: "홍콩행 열차는 광저우동역과 광저우남역 중 어디서 출발하나요?", answer: "두 역 모두 12306의 국경 통과 상품에 나오지만 직통 좌석은 날짜마다 다릅니다. 광저우남역은 판위, 광저우 남부, 철도 연계 이동에 맞고, 광저우동역은 당일 직통 열차가 있을 때 긴 시내 이동을 줄여 줍니다. 열차 시간을 비교하기 전에 표에 적힌 전체 역 이름을 호텔과 맞추세요." },
      { question: "광저우–홍콩 이동은 실제로 얼마나 걸리나요?", answer: "시간표의 탑승 시간보다 깁니다. 호텔 체크아웃부터 올바른 역 입구까지, 도보·보안검색과 운영사가 요구하는 사전 도착 시간, 철도나 도로 시간, 역 또는 출입경 지점의 출입경 절차, 도착 홀 도보와 홍콩의 마지막 MTR·택시·버스 구간, 다음 고정 예약을 지킬 여유까지 세세요. 이 페이지는 고정 시간표나 운임을 싣지 않습니다." },
      { question: "서카오룽역은 모든 홍콩 호텔에 편리한가요?", answer: "아닙니다. 서카오룽은 카오룽에 유용하고 홍콩의 넓은 도시 네트워크로 이어지지만 역 건물과 이후 도보가 0은 아니며, 센트럴, 코즈웨이베이, 공항 쪽, 신계 주소는 각각 마지막 구간을 따로 계산해야 합니다." },
      { question: "직통 열차 대신 선전을 거쳐 가야 하나요?", answer: "직통 좌석이 없거나 선전이 실제로 일정에 포함될 때만입니다. 선전을 거치면 표가 한 장 더 필요하고 역이나 국경 환승이 추가되며 연결을 놓칠 가능성이 늘어납니다. 그렇게 간다면 이름이 정해진 출입경 지점 하나를 고르고 이동 전에 양쪽을 확인하세요." },
    ] },
