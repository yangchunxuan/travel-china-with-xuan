...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "창바이산 자주 묻는 질문", items: [
    { question: "창바이산역과 창바이산시역은 같은 역인가요?", answer: "아닙니다. 영문 이름은 비슷하지만 서로 다른 역입니다. 长白山站은 북쪽 경구와 츠베이구·얼다오바이허 숙박에 기본으로 맞고, 长白山西站은 서쪽 경구와 쑹장허·츠시구 숙박에 맞습니다. 중국어 역명을 전부 그대로 옮겨 적고 호텔 픽업 구역을 확인하세요." },
    { question: "경구 예약서에 北景区라고 적혀 있으면 어느 거점으로 가나요?", answer: "철도는 长白山站, 숙소는 二道白河镇 또는 池北区부터 찾으세요. 천지 사진만으로는 어느 쪽 입구인지 알 수 없으니 날짜가 표시된 경구 예약서에서 北景区를 그대로 옮겨 적습니다. 이동 당일에는 경구 운영 공지, 12306 또는 항공사 기록, 도로 날씨, 허가된 마지막 환승의 승차 지점을 다시 확인하세요." },
    { question: "서쪽 스키 리조트에 묵으면서 북쪽 경구를 하루 다녀올 수 있나요?", answer: "서쪽 도착은 그대로 두되, 북쪽 경구 방문일은 두 입구 사이의 짧은 이동이 아니라 별도의 지역 간 도로 이동으로 계산하세요. 악천후이거나 입장이 이르다면 전날 숙박을 츠베이로 옮기는 편이 낫습니다. 실제로 쓸 수 있는 첫 산행일을 지키기 위한 선택입니다." },
    { question: "북쪽 경구와 서쪽 경구는 한 공원의 두 입구인가요?", answer: "한 안내센터의 서로 다른 문이 아니라 별도로 운영되는 두 지역입니다. 서쪽 관광 구역을 예약했다면 창바이산시역 또는 창바이산공항과 쑹장허·츠시구 숙박을 비교하는 편이 산악 지역을 가로지르는 이동을 줄여 줍니다. 교통 거점보다 관광 구역을 먼저 고르세요." },
    { question: "경구 입장권이 있으면 정상 구간도 운행하나요?", answer: "아닙니다. 강풍, 눈, 결빙, 시야 때문에 경구 내부 교통이 바뀌거나 일부 구간이 중단될 수 있고 겨울 운송 방식도 시즌마다 달라집니다. 전날 밤에 창바이산 공식 공지와 중국기상국 예보·경보를 확인하고, 호텔에서 나서기 직전에 한 번 더 확인하세요." },
    { question: "서쪽 경구를 예약했는데 창바이산역에 도착했다면 어떻게 하나요?", answer: "츠베이 쪽에 머물면서 허가된 지역 간 차량과 경구 상태부터 확인하세요. 더 안전한 방법은 서쪽 방문을 다른 날로 옮기거나 같은 쪽 숙박으로 바꾸는 것입니다. 시간에 쫓겨 무허가 차량을 타거나 원래 일정을 억지로 같은 날에 맞추지 마세요." },
  ] },
