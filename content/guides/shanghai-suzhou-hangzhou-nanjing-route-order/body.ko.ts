...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "group-size-heading", type: "heading", level: 3, text: "4명이나 6명이라면 숙박과 짐도 함께 비교하세요" },
  { id: "group-size", type: "table", caption: "공개 시작가 기준 구성. 결제 전 실제 날짜와 인원을 확인합니다", columns: ["확인할 점", "5일: 상하이·쑤저우", "6일: 상하이·쑤저우·항저우"], rows: [
    ["객실", "4명은 객실 2개, 6명은 3개. 상하이의 같은 호텔에서 4박합니다.", "4명은 도시마다 객실 2개, 6명은 3개. 상하이에서 쑤저우, 다시 항저우로 호텔을 두 번 옮깁니다."],
    ["짐과 도시 간 이동", "쑤저우 방문 중 큰 짐은 상하이 호텔에 둡니다. 4명과 6명도 왕복 고속철도 2등석과 양쪽 전용 픽업을 이용합니다.", "짐을 가지고 도시를 옮깁니다. 4~9명은 보통 문 앞까지 가는 차량을 계획하지만 실제 짐 수량에 맞는 차종을 확인해야 합니다."],
    ["견적 전", "모든 여행자의 나이, 침대 구성, 쑤저우 열차와 전체 일행의 총액을 확인합니다.", "세 도시 호텔, 차량 적재, 상하이 도착·항저우 출발과 전체 총액을 확인합니다. 상하이 복귀는 별도 계획입니다."],
  ] },
  {"id":"faq","type":"faq","title":"동선 순서 자주 묻는 질문","items":[{"question":"난징, 쑤저우, 상하이, 항저우를 어떤 순서로 도나요?","answer":"항공편이 서로 다른 관문을 쓸 수 있다면 난징→쑤저우→상하이→항저우 또는 그 반대가 실용적입니다. 장거리 왕복이 모두 상하이라면 한 방향이라는 이름에 매달리지 마세요. 상하이를 거점으로 삼고 전략적인 두 번째 거점을 더하거나, 전체 문간 이동 시간을 비교한 뒤 귀환을 받아들이면 됩니다."},{"question":"쑤저우는 당일 방문이 좋나요, 1박이 좋나요?","answer":"우선순위가 집중되고 쓸 만한 역이 활동 지역과 맞으며 고정 귀환이 방문을 일찍 자르지 않으면 당일 방문이 맞습니다. 현지 이른 시간이나 저녁이 중요하거나 우선순위가 여러 지역에 흩어져 있거나 다음 전진 열차가 상하이 귀환을 없앤다면 숙박하세요. 유명하다는 이유만으로 1박을 더하지 마세요."},{"question":"도시 하나를 빼야 한다면 어디를 빼나요?","answer":"고유 목적이 가장 약한 도시를 빼세요. 철도 구간이 짧아 보인다는 이유만으로 넣은 난징이 흔히 첫 삭제 대상입니다. 네 이름을 지키려고 모든 도시를 도착 저녁과 서두른 아침으로 줄이지 마세요. 남기는 도시에는 삭제 후에도 남을 고유 목적이 있어야 합니다."},{"question":"쑤저우에는 역이 하나뿐인가요?","answer":"아닙니다. 쑤저우, 쑤저우북과 새 남부 통로 역은 서로 바꿀 수 없습니다. 쑤저우 교통 당국은 상하이–쑤저우–후저우 노선과 함께 쑤저우남역·성쩌역이 개통해 기존 중심역·북역 밖의 선택이 늘었다고 기록합니다. 숙소와 활동 옆에 정확한 역명을 쓰고 여행일의 역 조합을 12306에서 검색하세요."},{"question":"항공편이 모두 상하이인데 꼭 돌아와야 하나요?","answer":"반드시 그렇지는 않습니다. 같은 문간 이동 계산으로 상하이 거점과 두 거점 혼합을 먼저 검사한 뒤 반복 철도 통근이나 마지막 공항 귀환을 받아들일지 정하세요. 두 거점은 이동 한 번으로 지리 이익 대부분을 얻으면서 네 번의 체크인을 만들지 않습니다. 마지막 상하이 귀환이 취약하면 오픈조 관문이나 마지막 상하이 1박을 비교하세요."},{"question":"도시를 옮기는 날 오후에 시간 지정 입장권을 예약해도 되나요?","answer":"이동일 도착 뒤에는 대체 불가한 시간 지정 활동을 두지 마세요. 체크아웃, 짐 이동, 역 접근과 지연 가능성이 모두 그 앞에 놓입니다. 당일 방문이 절반을 역 접근에 쓰면 가장 높은 우선순위 하나만 남습니다. 고정된 예약은 열차로 시작하지 않는 날에 넣으세요."}]},
