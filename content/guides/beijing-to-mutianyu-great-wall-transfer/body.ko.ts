...
const body = {
  ...
  blocks: [
    ...
    { id: "faq", type: "faq", title: "무톈위 이동 자주 묻는 질문", items: [
      { question: "베이징에서 무톈위까지 대중교통으로 어떻게 가나요?", answer: "베이징 공식 관광 정보는 현재 둥즈먼에서 916 급행버스로 화이러우베이다제까지 간 뒤 무톈위 방향 H노선 버스로 갈아타는 방법을 안내합니다. 노선, 정류장, 시간은 바뀔 수 있으니 중국어 정류장명을 저장하고 당일 귀환을 확인하며 화이러우에서 오래 기다릴 가능성도 계산에 넣으세요." },
      { question: "베이징에서 무톈위로 가는 직행버스가 있나요?", answer: "날짜가 정해진 관광 직행버스 상품으로만 있습니다. 승하차 지점과 귀환 시각이 호텔·관람 시간과 맞으면 환승을 한 번 줄여 주지만 출발지와 귀환이 모두 고정입니다. 예약 전에 공식 운영사, 정확한 정류장, 포함 사항과 날짜를 확인하세요." },
      { question: "버스나 차량 요금에 입장권과 케이블카가 포함되나요?", answer: "별도 구매층으로 보세요. 버스나 차량은 보통 도로 이동만 해결하고, 관광지 입장과 내부 셔틀, 케이블카·리프트·도보 접근은 각각 다른 규정과 대기가 있을 수 있습니다. 교통 판매 사진에 성벽이 나온다는 이유로 모든 표가 포함된다고 가정하지 마세요." },
      { question: "무톈위에 차량을 미리 예약하는 게 나을까요?", answer: "대체로 그렇습니다. 가족, 부모님, 빠듯한 일정이나 당일 항공편·열차가 있다면 전용차가 주는 안전 여유가 단순한 시간 절약보다 중요합니다. 시간이 유연하고 짐이 가벼운 여행자는 공식 대중교통을 이용할 수 있고, 날짜가 정해진 관광 직행버스는 승하차 지점과 귀환 시각이 맞을 때 그 중간이 됩니다." },
      { question: "무톈위에 다녀온 뒤 같은 날 저녁 열차를 탈 수 있나요?", answer: "통제된 차량을 택하거나 당일 연결을 없애는 편이 낫습니다. 정체, 역 혼동, 보안검색이 여유를 없애기 때문입니다. 연결을 유지한다면 더 이른 하산 시각을 정하고 도심 마지막 구간을 따로 계산하세요. 버스 도착을 역 도착으로 보면 안 됩니다." },
      { question: "직행버스를 놓치거나 화이러우에서 잘못 내리면 어떻게 하나요?", answer: "먼저 운영사의 공식 변경 규정과 현재 대중교통 노선을 확인하세요. 정류장을 잘못 내렸다면 저장해 둔 중국어 목적지를 직원에게 보여 주고, 방향을 확인하지 않은 채 노선 번호만 보고 타지 마세요. 이름 없는 차량을 권하는 호객은 따라가지 말고, 귀환편이 위험해지면 관광지를 나와 다음 확인된 교통을 확보하세요." },
    ] },
