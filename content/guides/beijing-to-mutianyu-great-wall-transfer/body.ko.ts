...
const body = {
  ...
  blocks: [
    ...
    { id: "quick-answer", type: "callout", title: "베이징을 떠나기 전에 돌아올 길부터", tone: "decision", body: "시간이 유연하고 짐이 가벼운 여행자는 공식 대중교통 연결을 이용할 수 있다. 날짜가 정해진 관광 직행버스는 승하차 지점과 귀환 시각이 맞을 때 환승을 줄인다. 가족, 부모님, 빠듯한 일정이나 당일 항공편과 열차가 있다면 전용차의 복구 가치가 표면적 시간 절약보다 클 수 있다." },
    ...
    { id: "decision-matrix", type: "table", caption: "전체 이동과 마지막 안전 귀환을 비교한다", columns: ["선택", "맞는 경우", "부담", "반드시 확인"], rows: [
      ...
      ["미리 정한 차량", "가족, 부모님, 시간 제한 또는 고정 후속 이동", "높은 비용과 만남 절차", "합법 차량, 승차 위치, 대기 조건과 아동 좌석"],
    ]},
    ...
    { id: "clock-heading", type: "heading", level: 2, text: "하루에 다섯 개의 시계를 둔다" },
    ...
    { id: "people-comparison", type: "comparison", title: "동행에 맞는 연결 선택", columns: [
      { heading: "독립적인 성인", items: ["대중교통도 합리적", "충전된 전화와 중국어 정류장 준비", "복구 시간 확보"] },
      { heading: "아이 또는 부모님", items: ["서서 기다리는 시간 줄이기", "환승 때 일행 유지", "화장실과 날씨 피할 곳 계획"] },
      { heading: "짐 또는 후속 열차", items: ["관광 이동에 여행 가방을 가져가지 않기", "출발 전 합법 보관", "같은 날 저녁의 빠듯한 연결 피하기"] },
    ]},
    { id: "late-heading", type: "heading", level: 2, text: "늦은 출발은 관람보다 선택지를 먼저 줄인다" },
    { id: "late-table", type: "table", caption: "복구 결정", columns: ["문제", "첫 행동", "하지 말 것"], rows: [
    ...
    { id: "sources", type: "sources", title: "공식 출처와 사진 표기", items: [
      ...
      { label: "대표 사진: Lloyd Tudor의 무톈위, CC BY-SA 4.0, 일부 자름", url: "https://commons.wikimedia.org/wiki/File:The_Mutianyu_section_of_the_Great_Wall_of_China.jpg", publisher: "Wikimedia Commons", reviewedAt: "2026-08-12" },
    ]},
  ],
