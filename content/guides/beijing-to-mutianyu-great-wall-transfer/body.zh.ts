...
const body = {
  ...
  blocks: [
    ...
    { id: "quick-answer", type: "callout", title: "离开北京前先选回程", tone: "decision", body: "时间灵活、行李轻的独立旅行者可使用官方公共交通链；日期明确的旅游直通车，只有上下车点和回程时间合适时才真正省事。家庭、父母同行、时间紧或当晚还要赶航班火车时，预先安排车辆的恢复价值往往高于表面节省的车程。" },
    { id: "matrix-heading", type: "heading", level: 2, text: "三种方式前往同一段长城" },
    { id: "decision-matrix", type: "table", caption: "比较完整接驳与最后一条安全回程", columns: ["方案", "通常适合", "摩擦", "必须核对"], rows: [
      ["公共交通链", "成年人、轻装、时间灵活且白天回程", "市区出发点、怀柔换乘、等车和站名混淆", "当前线路、运营时段和最后可用回程"],
      ["旅游直通车", "当天上下车点与游览时长正好匹配", "固定出发点与固定回程", "官方运营方、准确站点、包含项目与日期"],
      ["预先安排车辆", "家庭、父母、时间有限或后续有固定交通", "成本较高与会合安排", "合规车辆、上车定位、等待条款与儿童座椅"],
    ]},
    { id: "public-heading", type: "heading", level: 2, text: "官方公交是一条链，不是一班车的承诺" },
    ...
    { id: "clock-heading", type: "heading", level: 2, text: "一天放进五只时钟" },
    { id: "clock-list", type: "list", ordered: true, items: ["酒店到正确的北京出发点。", "去程车辆与每次计划换乘。", "下车点到真正登城入口。", "城墙上的时间、下山与集合。", "最后一条安全回程链，而不只是末班车发车时间。"] },
    { id: "people-heading", type: "heading", level: 2, text: "行动能力会改变交通答案" },
    { id: "people-comparison", type: "comparison", title: "让接驳符合同行人", columns: [
      { heading: "独立成年人", items: ["公共交通可以合理", "带好电量与中文站名", "预留恢复时间"] },
      { heading: "儿童或父母", items: ["减少站立和不确定等待", "换乘时全组同行", "计划洗手间与避雨处"] },
      { heading: "行李或后续火车", items: ["不要带箱子走观光链", "出发前合规寄存", "避免同晚脆弱衔接"] },
    ]},
    { id: "late-heading", type: "heading", level: 2, text: "晚出发会先减少选项，不只是缩短游览" },
    { id: "late-table", type: "table", caption: "恢复动作", columns: ["问题", "第一步", "不要"], rows: [
      ["错过直通车", "查运营方官方退改与当前公交链", "跟随不明司机乘坐未确认车辆"],
      ["到了错误怀柔站", "向工作人员展示保存的中文目的地", "只看线路号、不确认方向就上车"],
      ["回程有风险", "离开景区并锁定下一段已确认交通", "无备选地等待最后一班"],
      ["恶劣天气或关闭", "查看景区与北京官方通知", "把交通票当成开放证明"],
    ]},
    { id: "fact-check", type: "callout", title: "动态交通信息核验于2026年8月12日", tone: "neutral", body: "北京官方游客页面目前说明东直门、怀柔与慕田峪的公交链。线路号、站点、运营时间、旅游巴士产品、景区时间和夜游日期均可能变化，出发前要重查。" },
    { id: "help-callout", type: "callout", title: "需要把接驳放进你的北京行程？", tone: "decision", body: "提供日期、酒店、人数、行动需求、行李，以及当晚固定火车或航班。Homeground可以找出能接上的链路与复核点，但不承诺实时路况或景区开放。" },
    { id: "internal-links", type: "internal-links", title: "让北京行程接得起来", items: [
      ...
      { label: "外国游客去故宫", href: "/zh/guides/forbidden-city-for-foreign-visitors/", description: "把另一个高摩擦的北京日单独规划。" },
