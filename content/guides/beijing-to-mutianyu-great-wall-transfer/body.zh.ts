...
const body = {
  ...
  blocks: [
    ...
    { id: "faq", type: "faq", title: "慕田峪交通常见问题", items: [
      { question: "坐公共交通怎么去慕田峪？", answer: "北京官方游客信息目前写的是：东直门乘916快车到怀柔北大街，再换开往慕田峪方向的H线公交。线路、站点和时间都会变，所以要把中文站名存下来、核对当天回程，并把怀柔换乘的较长等待当成可能发生的事。" },
      { question: "有从北京到慕田峪的直通车吗？", answer: "只有按日期发布的旅游直通车，没有固定长期线路。上下车点和回程时间跟酒店、游览时长匹配时，它能省掉一次换乘，但出发点和回程都是固定的。订之前先核对官方运营方、准确站点、包含项目和出行日期。" },
      { question: "车票包含景区门票和缆车吗？", answer: "要当成不同的购买层。巴士或车辆通常只解决公路那一段；景区门票、内部接驳，以及缆车、索道或步行上山可能各有规则和排队。不能因为交通卖家的图片里出现长城，就默认所有票都包含在内。" },
      { question: "去慕田峪值得预先安排车辆吗？", answer: "多数情况下值得。家庭、父母同行、时间紧，或者当晚还要赶航班火车时，预先安排车辆带来的确定性往往比表面省下的车程更重要。时间灵活、行李轻的独立旅行者用官方公交就行；日期明确的旅游直通车介于两者之间，上下车点和回程合适时才真正省事。" },
      { question: "当天去慕田峪，晚上还能赶火车吗？", answer: "建议改用可控车辆，或者干脆取消当天的衔接。拥堵、车站弄错和安检会把余量吃掉。如果一定要保留，就把离开城墙的时间提前，并把市内最后一段单独计时，不要把巴士到站当成已经到了车站。" },
      { question: "错过直通车或下错怀柔站怎么办？", answer: "先查运营方的官方退改规定和当下的公交路线。站点搞错时，把存好的中文目的地给工作人员看，不要只看线路号、不确认方向就上车。不要跟着不明司机上未确认的车；如果回程本身有风险，就离开景区，锁定下一段已确认交通。" },
    ] },
