# Research and source log

**COPY READY — ASSETS OR CENTRAL FIX NEEDED**

Reviewed 2026-08-12 (Asia/Shanghai).

## Task and search review

Checked current results and questions for foreign-passport tickets, the official booking channel, travel from Chengdu, gallery order and visit duration. This page owns the whole museum-day execution chain; it is not a catalogue of Sanxingdui objects or a general Chengdu itinerary.

## Evidence boundary

- The museum website is the primary live notice and booking origin.
- Guanghan government's New Museum notice supports official channels, real-name entry and the current venue, but its trial-period hours and prices are not carried forward as permanent 2026 facts.
- Xinhua's inbound-tourism report records passport/other inbound-document booking, an inbound ticket pool and English payment pages.
- Exact release time, inventory, hours, extended service, transport departures and open galleries are dynamic and must be rechecked. The gallery order and three-hour allowance are editorial planning judgments.

## Pre-publication trigger

Open the live official ticketing flow and newest dated notice. Confirm passport fields, payment, release window, price, hours, last entry, gallery closures and any current Chengdu direct service. Update EN/ZH/KO together.
