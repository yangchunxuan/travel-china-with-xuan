...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "Questions about a rail-only route", items: [
    { question: "Should I rule out domestic flights and go rail-only in China?", answer: "Not before checking the weakest link. A rail-only route works when the cities form a continuous corridor, each segment has a useful station-to-station option, and the longest transfer does not consume the trip's core experience. A single flight at the edge can protect an otherwise coherent rail spine without turning the trip into an airport chain." },
    { question: "We have large suitcases. Does that change whether a rail-only route works?", answer: "Yes, luggage is one of the tests a rail segment has to pass. The rail-friendly result is a group that can board, store and move bags without losing control; the warning result is a transfer depending on escalators, tight timing or more bags than hands. Check that on the longest or least direct segment before refining the easy ones." },
    { question: "Does a through journey ticket on 12306 mean the connection is easy?", answer: "No. The current rules define a through journey ticket as separately purchased segments that can connect through the same or same-city passenger stations; that definition does not make every connection comfortable. Search the exact date and exact station, then add urban access, security, boarding, transfer and arrival time." },
    { question: "Does an overnight sleeper really save a hotel night?", answer: "Only on paper until you test the rest. The gain is moving while you would otherwise sleep; the costs to test are bed type, actual sleep, late boarding, early arrival and luggage before check-in. A sleeper that saves a hotel night on paper but destroys the next day is a warning result, not a saving." },
    { question: "A rail segment is disrupted mid-trip. What should I do first?", answer: "Confirm the service status with 12306 or station staff, keeping the ticket and the document used for purchase available, then protect the next fixed departure. Optional sightseeing should not compete with the final viable connection. If the chain no longer works, stop the cascade: retain the current room or arrange one night and rebuild from confirmed services." },
    { question: "How should I count the time a train segment really takes?", answer: "Door to door, not the scheduled journey length. Convert each train into a block that includes checkout, local transport, station margin, rail, arrival transport and the check-in gap. Then check whether that block fits naturally into a move day or removes most of a scarce sightseeing day." },
  ] },
