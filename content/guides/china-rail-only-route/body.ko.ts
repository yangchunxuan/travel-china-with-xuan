...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "동선 관련 자주 묻는 질문", items: [
    { question: "중국에서는 국내선을 아예 빼고 철도만 타야 하나요?", answer: "가장 약한 연결을 확인하기 전에 ‘국내선 비행 없음’을 규칙으로 만들지 마세요. 도시가 연속된 통로를 이루고, 모든 구간에 실용적인 역 대 역 선택이 있으며, 가장 긴 이동이 여행의 핵심 경험을 삼키지 않을 때 철도 전용 동선이 작동합니다. 가장자리에서 항공편을 한 번 쓰면 여행을 공항 연속으로 만들지 않고도 중앙의 일관된 철도 축을 지킬 수 있습니다." },
    { question: "짐이 많은데 철도 전용 동선이 가능할까요?", answer: "그렇습니다. 짐은 철도 구간이 통과해야 하는 검사 가운데 하나입니다. 철도에 맞는 결과는 일행이 통제력을 잃지 않고 승차·보관·이동을 할 수 있는 상태이고, 경고 결과는 환승마다 에스컬레이터, 빠듯한 시간 또는 손보다 많은 짐에 기대야 하는 상태입니다. 가장 길거나 덜 직접적인 구간에서 이 점을 먼저 확인한 뒤 쉬운 구간을 다듬으세요." },
    { question: "12306의 연계 승차권이면 환승이 편하다는 뜻인가요?", answer: "아닙니다. 현행 규정은 연계 승차권을 같은 역 또는 같은 도시의 여객역에서 이어 탈 수 있도록 구간별로 구매한 여러 표로 정의하지만, 그 정의가 모든 연결을 편안하게 만들지는 않습니다. 정확한 날짜와 역을 검색하고 도시 접근, 보안, 승차, 환승과 도착 시간을 더하세요." },
    { question: "야간 침대열차를 타면 정말 숙박비를 아끼나요?", answer: "종이 위에서만 그렇고, 나머지 비용을 먼저 검사해야 합니다. 이익은 원래 잘 시간에 이동한다는 점이고, 검사할 비용은 침대 유형, 실제 수면, 늦은 승차, 이른 도착과 체크인 전 짐입니다. 숙박은 아끼면서 다음 날을 망치는 침대열차는 절약이 아니라 경고 결과입니다." },
    { question: "여행 중 철도 구간에 문제가 생기면 무엇부터 하나요?", answer: "12306이나 역 직원에게 운행 상태를 확인하고 승차권과 예약에 사용한 신분증을 준비한 뒤, 다음 고정 출발을 먼저 보호하세요. 선택 관광이 마지막 가능한 연결과 경쟁해서는 안 됩니다. 연쇄가 더 이상 이어지지 않으면 중단하고, 현재 방을 유지하거나 1박을 마련한 뒤 확인된 다음 날 운행부터 다시 만드세요." },
    { question: "열차 한 구간에 시간을 얼마나 잡아야 하나요?", answer: "운행 시간이 아니라 전체 문간 이동으로 계산하세요. 체크아웃, 현지 교통, 역 여유, 철도, 도착 교통과 체크인 공백을 합쳐 각 열차의 전체 이동 시간을 내세요. 그런 다음 그 이동이 이동일에 자연스럽게 들어가는지, 희소한 관광일 대부분을 없애는지 확인하세요." },
  ] },
