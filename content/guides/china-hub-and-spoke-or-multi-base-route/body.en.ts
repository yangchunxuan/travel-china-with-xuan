...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "Questions travellers ask when shaping a route", items: [
    { question: "Should I stay in one city and take day trips, or change hotels as I go?", answer: "Use one hub when the spokes are genuinely short door to door and hotel stability saves more effort than repeated returns cost. Use multiple bases when the route moves forward and each hotel change unlocks several priorities. For many China trips the strongest answer is a hybrid: one stable regional hub, then one deliberate move to the next cluster." },
    { question: "The train is short, so is it an easy day trip?", answer: "Not by itself. The timetable cannot decide the architecture. Add hotel checkout, transport to the correct station, security and boarding margin, the train, arrival transport, luggage handling and, on a spoke, the fixed return. A short rail segment can still consume both ends of a day. Verify the current station pair on China Railway 12306 once your dates are known." },
    { question: "Is one night in an extra city worth the hotel change?", answer: "Only when that base unlocks at least two priorities or removes a major return. A base that serves one optional sight costs more in checkout than it gains, so visit it as a focused stop or remove it. One-night hotels between every attraction mean the route is using hotels as punctuation; merge neighbouring priorities into fewer bases." },
    { question: "What happens to our bags on a hotel move day?", answer: "Decide it before booking: a move day may leave hours between checkout and room access, so plan where the bags and the tired travellers go. Count who can move each bag while handling passports, children or mobility needs, and recheck each hotel's luggage policy. Checkout, station access and luggage all land on the same day." },
    { question: "Can we keep a timed booking on the day we change hotels?", answer: "Better not. A move day that also contains a timed highlight leaves delay and luggage no recovery space, so protect the highlight on a full local day instead and keep move days free of any irreplaceable immediate booking. On a move day a delay can cascade into the hotel, the transfer and later reservations." },
    { question: "Our day trip uses a station far outside the city. Does that change the answer?", answer: "It can reverse the decision. Write the exact station, not only the city: a service at a distant terminal changes the whole door-to-door block. On an out-and-back day trip the outbound and return both consume fixed time, and you repeat the hotel-to-station approach plus departure margin on every excursion. That is why the exact station belongs beside every rail segment before you assign nights." },
  ] },
