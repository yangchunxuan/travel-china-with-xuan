...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "路线结构常见问题", items: [
    { question: "该住一个城市当天往返，还是一路换酒店？", answer: "支线短就守一个基地，路线向前就换。只有往返支线门到门耗时确实短，固定酒店省下的精力大于重复折返，才适合单一基地；路线持续向前、每次换酒店都能解锁多个核心项目，就适合多基地。很多中国路线其实适合混合结构：先用一个稳定的区域基地，再有意识地移动一次进入下一个集群。" },
    { question: "列车运行时间短，是不是就适合当天往返？", answer: "不一定。时刻表本身决定不了路线结构。还要加上酒店退房、前往正确车站、安检与候车余量、列车、抵达后的市内交通、行李处理，支线还有不可避免的返程。一段很短的铁路，照样可能占掉一天的两头。日期定了以后，在中国铁路12306核对实际车站组合和当日服务。" },
    { question: "为了一个景点多换一次酒店值得吗？", answer: "只有当这个基地至少解锁两个重点，或者省掉一次重大折返，才值得。只服务一个可选景点的基地，退房的代价大于收获，不如从稳定基地集中访问，或者干脆删掉。如果每个景点之间都冒出一晚酒店，说明酒店被当成了路线标点，应该把相邻的重点合并到更少的基地里。" },
    { question: "转场当天行李怎么办？", answer: "订房之前就要定好。转场日可能在退房后、入住前空出好几个小时，先想清楚行李和已经很累的人待在哪里。再算一算：谁腾得出手拿每一件行李，同时还要顾护照、孩子或行动需求？每家酒店的寄存政策也要逐一确认。退房、进站和全程带着行李，往往挤在同一天。" },
    { question: "换酒店那天可以安排定时项目吗？", answer: "尽量别安排。转场日再加一个不可替代的定时项目，延误和行李就没有应变余量，核心项目应该放在不转场的完整游览日。转场日的延误还可能连锁影响酒店、接驳和后续预订，所以抵达之后也不要紧接不可替代的预约。" },
    { question: "一日游的车站在远郊，结论会变吗？", answer: "可能整个反过来。要写下准确车站，而不只是城市：服务如果使用远郊车站，整段门到门的时间就变了。支线的去程和返程都占固定时间，酒店到车站的路程和候车余量每次出游还要重复一遍。所以分配住宿夜数之前，先把准确的出发站和到达站写在每段铁路旁边。" },
  ] },
