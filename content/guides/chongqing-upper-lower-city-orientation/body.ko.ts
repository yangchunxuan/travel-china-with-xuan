...
const body: StructuredPageBody = {schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "길 찾기 자주 묻는 질문", items: [
    { question: "지도에서 200m인데 충칭에서는 왜 가기 어려운가요?", answer: "두 길의 높이가 다르기 때문일 가능성이 큽니다. 가파른 지형에 건물이 밀집한 충칭에서는 평면 지도에서 가까운 두 길도 높이가 수십 미터 차이 날 수 있고, 그 사이에 곧바로 이어지는 계단이 없기도 합니다. 그래서 목적지는 가로 위치, 세로 높이, 그리고 번호가 있는 역 출구·엘리베이터·에스컬레이터·다리 같은 이용 가능한 연결로까지 세 가지로 확인해야 합니다." },
    { question: "리지바는 열차가 낡은 아파트를 뚫고 지나가는 건가요?", answer: "아닙니다. 충칭시 공식 설명에 따르면 리지바역과 건물은 서로 조율해 설계됐고, 철도 구조와 건물 구조를 분리해 진동과 소음을 관리합니다. 유명한 장면은 제약이 큰 지형과 고밀도 시가지에 대응한 공학적 해법이지, 나중에 낡은 아파트를 무심코 관통한 결과가 아닙니다." },
    { question: "리지바역은 공사 중인가요?", answer: "2026년 6월 13일 충칭시 공식 공지 기준으로는 그렇습니다. 다음 공사 단계에서 대합실·승강장·1번과 2번 출입구를 단계적으로 폐쇄하며 2026년 12월 30일 완공할 예정이라고 밝혔습니다. 날짜가 있는 공사 공지이지 영구 폐쇄가 아니므로, 역을 이용하거나 전망 동선을 계획하기 전에 충칭 철도교통의 최신 안내를 다시 확인하세요." },
    { question: "충칭에서 여행가방이 있으면 지도상 최단 경로로 가야 할까요?", answer: "대개는 아닙니다. 짐이 있거나 부모님과 함께이거나 이동에 제약이 있다면, 지도상 조금 더 멀더라도 엘리베이터, 에스컬레이터, 차량 접근 도로나 역 출구를 고르세요. 직선거리보다 높이 이동을 확실히 해결하는 편이 낫습니다. 엘리베이터 하나는 오르막 하나만 없애므로 도로 높이의 차량 이동도 대안으로 남겨 두세요." },
