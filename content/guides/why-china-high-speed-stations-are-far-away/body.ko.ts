...
const body: StructuredPageBody = {schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "역 선택 자주 묻는 질문", items: [
    { question: "중국 고속철도역은 왜 도심에서 먼가요?", answer: "철도 선형과 용량, 건설·철거의 제약, 지역 교통 연결, 때로는 도시 개발 목표까지 함께 조정하기 때문이며 무엇을 우선하는지는 도시마다 다릅니다. 국가 지침은 새 역을 시가지 안이나 가능한 한 가까이에 두도록 하므로 외곽 입지 자체가 계획의 성공을 뜻하지는 않습니다. 여행자에게 필요한 판단은 정확한 역에서 호텔까지 이어지는 전체 이동을 비교하는 것입니다." },
    { question: "역명의 ‘동·서·남·북’이 그렇게 중요한가요?", answer: "네, 서로 완전히 다른 역을 가리킬 수 있습니다. 공식 역명에 방향이나 다른 수식어가 붙어 있으면 12306 검색 결과나 예약 확인서에서 전체 이름을 그대로 복사하고, 가능하면 중국어 역명도 함께 호텔 경로와 택시 목적지에 넣으세요. 비공식 번역만으로 다른 역을 추정해서는 안 됩니다." },
    { question: "큰 짐이 있고 일행이 천천히 걷는다면 역 선택이 달라지나요?", answer: "예매 전에 출발부터 도착까지 계단 없는 경로를 확인하세요. 개별 역에 엘리베이터가 있어도 모든 환승이 계단 없이 이어지지는 않고, 승강장 출구, 짐 이동, 환승 대기와 마지막 이동도 더 오래 걸립니다. 구도심 호텔, 가족 짐, 늦은 도착이 겹치면 도심역이나 더 느린 열차가 문 앞까지 계산하면 더 나을 수 있습니다." },
    { question: "불편한 역으로 예약했는데 같은 도시의 다른 역에서 탈 수 있나요?", answer: "같은 도시의 다른 역에서 그 승차권을 쓸 수 있다고 가정하지 마세요. 공식 12306 주문에서 현재 가능한 선택지를 확인하세요. 승차권을 유지한다면 정확한 역명과 호텔 이름·주소를 중국어로 저장하고 마지막 대중교통 종료 시각과 도로 이동 대안을 확인한 뒤 더 일찍 출발하세요. 도착 후 연결이 끊기면 역 직원에게 현재 허가된 택시 또는 차량 호출 승차 지점을 물어보세요." },
    { question: "열차가 막차 이후에 도착하면 어떻게 하나요?", answer: "심야에도 운행하는 교통편을 확인하거나 도로 이동 비용을 계산하세요. 도착 뒤에는 승강장 출구, 짐 이동, 환승 대기와 호텔까지 마지막 이동도 더해야 합니다. 지하철에서 긴 환승을 두 번 해야 한다면 더 도심에 있는 역 또는 직행 차량 이동과 비교하세요." },
    { question: "예매 전에 두 역을 어떻게 비교하나요?", answer: "실제 열차 검색 결과에서 출발역과 도착역의 전체 이름을 복사한 뒤, 호텔의 실제 입구에서 정확한 철도역까지 실제 지하철·버스 정류장이나 확인된 차량 승하차 지점으로 경로를 찾으세요. 역 중심 핀은 문 앞 기준점이 아닙니다. 호텔 체크아웃, 역 입장과 보안, 대합실 보행과 현실적인 여유 시간을 더합니다." },
  ] },
