...
const body = {
  ...
  blocks: [
    ...
    { id: "faq", type: "faq", title: "购票前常见问题", items: [
      { question: "大索道能从市区直接坐到山顶吗？", answer: "不能。索道改造期间，大索道中站到山顶上站这一段仍然停运，A线要在中站换景区游览车去天门洞。按红网刊载、2026年8月31日起执行的景区公告（9月13日复核），双程索道A、B、C线开放，单程索道1、2线继续暂停。" },
      { question: "扶梯包含在票里吗？", answer: "只有一段包含。套票里的7级穿山扶梯连接天门洞后方和山顶，上下都含在内。天门洞前999级台阶旁的“登天门扶梯”是另一段：公告所述施工安排中，下行免费，上行仍要另外付费。山顶观光小缆车和玻璃栈道的服务费也要分开核对。" },
      { question: "天门山几点停止售票？", answer: "8月31日公告的售票时间是7:30—16:00，运营从8:00开始，同程公告另列16:00停止检票。请按已订时段到场。预约不等于到了就能上索道，16:00也不是游览结束的时间，所以当天最好别紧接时间很赶的火车或航班。" },
      { question: "用护照订的票，当天要带什么、在哪检票？", answer: "带预约时用的护照原件，再加上门票本身。同程所载公告把护照列为人工核验身份的证件之一，同时需要出示电子或纸质门票，所以门票要离线存好。取票和检票按自己订单的说明走；需要换票的，提前确认位置，别默认所有外籍护照订单都在同一个窗口。" },
      { question: "72元是天门山的全部费用吗？", answer: "不是。72元是2025年8月1日起执行的门票政府指导价，湖南省发改委通知规定的只是门票项目，不含索道、游览车和扶梯。同程所载施工期公告列明，A、B、C线普通成人套票执行价为285元，资料复核日期是2026年9月13日，最终支付金额仍要向卖家核对。" },
      { question: "这张票包含张家界国家森林公园或玻璃桥吗？", answer: "不包含。张家界国家森林公园、大峡谷玻璃桥都另有各自的门票。下单时确认商品是“天门山国家森林公园”，日期、A/B/C线路和入园时段都对，并确保付款后能取得实际门票或入园码，而不是只剩一张支付收据。" },
    ] },
