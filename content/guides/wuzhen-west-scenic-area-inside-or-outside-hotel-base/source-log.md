...
- `guide:generate` and `guide:check`: **passed** for 178 independent guide folders. `content:check`: **passed**, reporting 0 manifest entries from the repository's current manifest input; this does not independently prove route publication. Planning-scope check and TypeScript `tsc --noEmit`: **passed**. Entity audit exited 0 and resolved this guide only to `country-china`, while reporting 94 guides with pre-existing unmapped tokens across the repository. `git diff --check`: **passed** before final staging and must be repeated against the staged patch.
