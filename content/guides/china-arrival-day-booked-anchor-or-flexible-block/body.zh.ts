  ...
  blocks: [
    {
      id: "lead-answer",
      type: "lead",
      text: "国际抵达日默认应保持弹性。只有入境手续、行李、地面交通、酒店衔接和场馆入场这几环都已核清，而且错过预约的代价可以接受，才考虑安排稀缺的定时项目。任何一个关键输入未知，都应改用可替换时段，而不是把计划落地或到站时间直接当成游览开始时间。"
    },
    {
      id: "two-blocks",
      type: "comparison",
      title: "先分清第一天放进去的是什么",
      columns: [
        { heading: "定时锚点", body: "绑定姓名、日期或入场时段的稀缺预约，并有迟到、取消或爽约后果。它不能随意往后挪两个小时。" },
    ...
    { id: "gate-heading", type: "heading", level: 2, text: "定时锚点必须同时通过四道准入门槛" },
    {
      id: "anchor-gates",
      type: "list",
      ordered: true,
    ...
    { id: "branches-heading", type: "heading", level: 2, text: "三种抵达方式会得出不同答案" },
    {
      ...
      columns: [
        ...
        { heading: "国内航班", items: ["没有边检，但登机口、行李和地面交通仍会变化", "航班异常还可能改变司机等待条件", "通常用可替换的晚上，比稀缺门票更稳妥"] },
        { heading: "国内铁路", items: ["具体车站、证件流程和出口仍然重要", "只带随身行李可删去托运行李分支", "是否能放较晚锚点，取决于实时 12306、酒店寄存和场馆规则"] }
      ]
    },
    {
      id: "venue-proof",
      type: "paragraph",
      text: "真实场馆能说明最后一道门为什么重要。故宫博物院公布实名、日期、时段和证件规则；中国国家博物馆要求按预约证件和时段核验，并设取消及爽约条件。它们只是“脆弱锚点”的具体例子，不代表所有博物馆规则相同。订票前和抵达当天，都要重新阅读目标场馆自己的通知。"
    },
    { id: "scenarios-heading", type: "heading", level: 2, text: "两类旅客，两种有证据的结论" },
    {
      ...
      columns: ["旅客", "链条检验", "决定"],
      rows: [
        ["下午抵达的长途国际航班、托运行李，想订当天故宫", "航班、边检、行李、进城、酒店和场馆安检是串联风险；门票无法吸收未知延误", "不要把故宫当抵达日锚点。改用附近用餐、散步或休息，并把参观放在完整本地日"],
    ...
    { id: "failure-heading", type: "heading", level: 2, text: "在哪个节点失败，就在那里止损" },
    {
      ...
      rows: [
        ...
        ["铁路抵达受影响", "按当前 12306 规则和实际余票处理；改签并非保证", "没有从容的后续链就取消锚点"],
    ...
    { id: "switch-heading", type: "heading", level: 2, text: "哪些条件会改变结论" },
    {
      ...
      type: "comparison",
      columns: [
        { heading: "更接近定时锚点", body: "国际抵达改为国内抵达；托运行李改为随身行李；酒店确认立即寄存；场馆出现明显更晚的时段；或预约易取消且损失低。" },
    ...
    },
    {
      ...
      tone: "neutral",
      title: "本页只拥有一个第一天决策",
      body: "本页判断抵达日能否承载稀缺定时锚点。“行程是否太赶”文章负责全程节奏与转场过载；机场和车站文章负责具体走法；离境前一晚文章负责回程保护。本页不计算完整私人行程，也不承诺入境、交通或入场时间。"
    },
    {
      id: "human-help",
      type: "callout",
