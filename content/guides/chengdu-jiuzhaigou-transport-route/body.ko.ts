...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "이동 자주 묻는 질문", items: [
    { question: "청두에서 기차를 타면 주자이거우 계곡까지 바로 가나요?", answer: "아닙니다. 황룽주자이는 기차역이지 계곡 입구가 아닙니다. 기차역, 주자이거우 숙소, 이용할 관광지 입구 세 위치를 따로 저장해 두세요. 한 곳에 도착하는 셔틀표가 나머지 두 곳까지 데려다주지는 않습니다." },
    { question: "황룽주자이역에서 관광지까지는 어떻게 가나요?", answer: "미리 예약한 후속 환승으로 갑니다. 주자이거우의 2026년 7월 공지는 황룽주자이역 환승을 주뤼웨싱(九旅悦行)에서 미리 예약하도록 안내하며, 열차 도착에 맞춰 연결되고 관광지 입구까지 약 2시간 걸린다고 설명합니다. 쑹판은 같은 채널에 해당 날짜 연결편이 있을 때만 대안이 됩니다." },
    { question: "비행기를 타면 열차보다 빠른가요?", answer: "실제 항공편과 이름이 확인된 도착 뒤 교통이 모두 있을 때만 그렇습니다. 공항 경로에도 약 88km의 남은 도로가 있고 공항 접근과 날씨, 짐까지 더해지므로 비행 시간이 아니라 전체 동선을 비교하세요. 날짜와 실제 청두 공항, 항공편, 픽업, 운항 중단 규정을 확인해야 합니다." },
    { question: "열차가 늦으면 환승 차량이 기다려 주나요?", answer: "그렇게 가정하지 마세요. 판매자가 보호를 글로 명시하지 않는 한 철도·항공 예약과 역에서 계곡까지 가는 환승은 서로 다른 계약이며, 열차 도착에 맞춰 배치된 정기 픽업은 전용차가 무기한 기다리는 것과 다릅니다. 변경·미탑승 조건을 읽고 정확한 열차를 운영사에 알린 뒤 충분한 여유를 두세요." },
    ...
    { question: "돌아오는 교통도 따로 계획해야 하나요?", answer: "별도의 연결로 계획하세요. 일반적인 '주자이거우'가 아니라 숙소나 관광지 출구에서 시작해, 도로 환승의 픽업 지점과 그 차량이 실제로 서비스하는 철도역·공항을 확인합니다. 별도 철도·항공권 앞에는 산길 중단에 대비한 여유를 두고, 항공편을 대체할 수 없다면 마지막 가능한 연결 대신 더 일찍 청두나 출발 공항 쪽으로 이동하세요." },
  ] },
