...
const body: StructuredPageBody = {
  ...
  blocks: [
    ...
    },
    {
      ...
      items: [
        {
          question: "航班、火车票和酒店订单上的姓名必须完全一样吗？",
          answer: "不必完全一样。航司、中国铁路 12306、酒店和景区显示的姓名未必一致，填写时按各自的字段说明来。真正必须一致的是旅客本人、证件类型、证件号码和姓名来源，也就是你旅行时实际携带的那本有效护照。",
        },
        {
          question: "在 12306 该怎么填写外国护照上的姓名？",
          answer: "先看护照上印的标签。印 Surname 和 Given Names，就按这个顺序填，中间留一个空格；印 First、Middle、Last Name，就按所列顺序用空格分隔。外国护照姓名只录入拉丁字母 A–Z 或 a–z；别的平台要求姓在前，就按它的规则填。",
        },
        {
          question: "12306 买完票后发现乘车人姓名填错了，还能改吗？",
          answer: "不能改。12306 现行购票问答写明，购票后不能更改乘车人姓名；乘车人身份信息填错时，官方给出的处理方式是退票后重新购买，并按规定收取退票费。改日期或车次修复不了身份错误，动手前先看清官方流程和当前余票。",
        },
        {
          question: "姓名太长，预订表单填不下，该删掉哪一段？",
          answer: "不要自己决定删哪一段。12306 目前允许录满 64 个字符，超出部分不补录，按平台写明的上限填即可，同时把规则和自己的输入保存成证据。如果截断后的姓名可能对应不止一个人，就找服务商处理，不要自创一种更短的拼写。",
        },
        {
          question: "酒店订单上的名字和护照不完全一样，能入住吗？",
          answer: "预订姓名和法定入住身份是两回事。预订姓名的作用是让酒店找到并保留房间；办理入住时，外籍住客需要出示有效护照、外国人永久居留身份证或其他认可证件，由酒店完成住宿登记。付款人、预订人和实际住客不是同一人时，要通过预订渠道添加住客的护照姓名，并在到店前问清酒店。",
        },
        {
          question: "订票后换发了新护照，订单会自动更新吗？",
          answer: "不会自动更新。新护照号码不会自动同步到既有订单，法定更名证明也不会自行改写客票。航司、中国铁路、酒店和景区之间既没有共享的更正数据库，也没有统一规则，所以要依法保留新旧证件信息和变更证明，再分别联系每个服务商。",
        },
      ],
    },
    {
