...
const body = {
  ...
  blocks: [
    ...
    { id: "faq", type: "faq", title: "选择出行时段常见问题", items: [
      { question: "去中国旅行，几月最好？", answer: "中国没有一个适合所有地区的“最佳天气月份”。跨区域旅行应先保护最容易被高温、降雨、严寒、能见度或海拔影响的核心体验，再看其余地区在同一时段是否可以接受。这套方法用于选择日期窗口，不是预测你出行那几天的天气。" },
      { question: "假期日期固定，应该改什么？", answer: "保留日期，调整路线。年假、校历或活动日期固定，某一个地区制造了大部分打包和恢复负担，或同行者无法承受最难暴露时，都应该这样做。只有整趟旅行围绕一个季节性户外体验展开、其他地区可以跟随时，才移动日期。两种选择都不是失败。" },
      { question: "可以直接看全国月平均值来定日期吗？", answer: "只能当筛选，不能当预报。全国月平均抹平了旅客真正会感受到的地区和海拔差异，多年平均也无法确认某一周的降雨或能见度。应比较路线里的具体暴露与最难妥协的活动，临近出发再查官方预报，并保留可移动的日程。" },
      { question: "行程里加一段高原，选期要注意什么？", answer: "先让最怕天气的那一段决定大致窗口，再判断同行者能否适应高原条件和恢复节奏。海拔要作为独立约束写进行程，而不是只写进打包清单；上高原的第一天要保持弹性，并想好低海拔退出方案。如果不适合，就删掉高原延伸段，而不是让一个日期同时满足三种不相容的暴露。" },
      { question: "天气合适是不是就可以订了？", answer: "还要再做两步核对。先依据官方长期气候背景，为最难妥协的一站筛出两三个大致窗口，再逐一检查其他地区是否仍然可接受；然后核对官方节假日日历和具体景区、交通运营公告，选择严重冲突最少的时段。只有会破坏核心体验或超过同行者耐受度时，才否决一个窗口。" },
      { question: "临近出发该看哪里的信息？", answer: "当前天气和预警看中国气象局和对应的地方气象部门，停运或闭园看具体景区、交通运营方。本文刻意不做“城市×月份”预报。短期预报可能改变每日顺序，但不一定推翻区域层面的选择逻辑。" },
    ] },
