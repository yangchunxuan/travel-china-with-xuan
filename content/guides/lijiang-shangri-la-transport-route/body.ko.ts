...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "direct-answer", type: "callout", title: "차량만이 아니라 도착 후의 상태를 고르세요", tone: "decision", body: "도시만 바로 옮긴다면 여행일의 12306에서 열차를 확인하고 호텔 픽업과 조용한 첫 저녁을 보호하세요. 실제 문전 이동이나 계획된 중간 정차의 이점이 있을 때만 도로를 고릅니다. 몸 상태, 날씨, 교통으로 원래 동선이 불가능하면 저녁 활동을 줄이고 다음 날 일정을 늦추거나 더 낮은 거점에 머물거나 돌아가세요. 체크리스트를 억지로 지키는 것이 목표가 아닙니다." },
  ...
  { id: "mode-matrix", type: "table", caption: "실시간 운행 횟수나 고정 도로 시간을 약속하지 않습니다", columns: ["방식", "잘 맞는 경우", "주요 부담", "결론이 바뀌는 조건"], rows: [
    ["여행일 열차: 丽江站→香格里拉站", "숙박 거점 바로 이동, 가족, 도시 간 구간을 통제하고 싶은 여행자", "양쪽 역 접근, 짐, 빠른 고도 상승", "적합한 재고 없음, 큰 운행 차질, 필수 중간 정차"],
  ...
  { id: "altitude", type: "comparison", title: "첫날 밤의 보호 범위를 만드세요", columns: [
  ...
  { id: "base", type: "table", caption: "‘샹그릴라 호텔’은 픽업 지시가 아닙니다", columns: ["거점 유형", "이동 지시", "첫날 밤 의미"], rows: [
    ...
    ["도심 밖 리조트·농촌 숙소", "픽업 범위, 차량 신원, 짐 용량, 늦은 도착 규칙을 서면 확인", "마지막 도로 구간이 그날 가장 복구하기 어려울 수 있음"],
    ["다음 날 관광 출발 지점", "역 픽업 및 호텔 주소와 별개로 확인", "도착 차량이 다음 날 투어로 자동 전환된다고 가정하지 않음"]
  ]},
  { id: "chain-heading", type: "heading", level: 2, text: "통제 가능한 인계로 이동을 실행하세요" },
  { id: "rail-chain", type: "list", ordered: true, items: [
    ...
    "픽업, 체크인, 휴식을 보호할 결과로 두고 저녁 식사·관광은 선택으로 남깁니다."
  ]},
  ...
  { id: "scenarios", type: "table", caption: "같은 열차도 서로 다른 첫날 밤을 만듭니다", columns: ["여행자", "실행 계획", "피할 것"], rows: [
    ...
    ["연세 있는 부모님과 가족 여행", "불확실한 인계가 가장 적은 경로, 짐은 차량과 함께, 객실 접근 방법 확인", "‘고성’이면 차가 문 앞까지 가고 평탄하다고 가정"],
  ...
  { id: "downgrade-heading", type: "heading", level: 2, text: "원래 계획이 실패했을 때의 단계별 축소" },
  { id: "downgrade", type: "list", ordered: true, items: [
    "선택적인 도착 저녁 활동을 먼저 빼고 확인된 호텔 픽업은 유지합니다.",
    "다음 날 오전 일정은 늦추고 휴식 후 다시 판단합니다.",
    "철도·도로가 끊기면 리장 객실 또는 통제 가능한 더 낮은 거점의 숙박을 유지합니다.",
  ...
  { id: "help", type: "callout", title: "열차, 호텔, 첫날 밤을 함께 점검할까요?", tone: "decision", body: "날짜, 리장 출발지, 샹그릴라 호텔, 인원, 짐, 대략적인 편안함 조건을 Homeground에 보내 주세요. 인계와 축소 여유를 검토할 수 있습니다. 개인 건강 조언은 의료진과 상의하고 교통·날씨 운영은 공식 사업자의 안내를 따르세요." },
  { id: "links", type: "internal-links", title: "다음 계획", items: [
    ...
    { label: "중국 일정이 너무 빠듯한가요?", href: "/ko/guides/is-your-china-itinerary-too-rushed/", description: "첫날 밤을 일정 압축에서 보호합니다." }
  ]},
