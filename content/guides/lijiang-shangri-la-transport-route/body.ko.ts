...
const body = { schemaVersion: "1.0.0", blocks: [
  ...
  { id: "faq", type: "faq", title: "출발 전 자주 묻는 질문", items: [
    { question: "리장에서 샹그릴라까지 열차와 전용 차량 중 무엇이 좋나요?", answer: "숙박 거점을 바로 옮길 때는 철도가 기본안입니다. 도로 노출을 줄이고 도시 간 구간을 관리하기 쉽습니다. 일부러 중간에 들르거나 정확한 호텔 위치 때문에 문에서 문까지 가는 차량이 뚜렷하게 유리할 때만 허가된 도로 이동이 더 낫습니다. 정하기 전에 12306에서 여행일 열차부터 확인하세요." },
    { question: "샹그릴라는 리장보다 얼마나 높나요?", answer: "역 고도가 리장역 약 2,400m에서 샹그릴라역 약 3,274m로 올라갑니다. 철도 도착은 이동 구간인 동시에 고도 변화이므로, 역에서 객실까지 어떻게 갈지와 첫날 밤을 얼마나 비워 둘지도 별개 문제가 아니라 같은 교통 계획에 들어갑니다." },
    { question: "샹그릴라까지 장거리 버스로 갈 수도 있나요?", answer: "현재 허가된 노선이고 여행일의 노선과 승차장이 맞을 때만 고려하세요. 예산 중심 일정에는 장거리 버스가 맞지만 터미널 접근과 짐 처리, 운영 변경이 부담입니다. 오래된 시간표만 근거라면 충분하지 않고, 도착 뒤 호텔 픽업을 놓치는 일정이라면 버스는 답이 아닙니다." },
    { question: "샹그릴라역에서 숙소까지 어떻게 가나요?", answer: "리장을 떠나기 전에 정해 두세요. 샹그릴라 호텔에 열차 기록과 인원, 짐, 연락 가능한 번호를 보내고, 역에서는 공식 픽업·택시 구역을 이용해 차량을 확인한 뒤 짐을 실으세요. 목적지는 막연한 고성 핀이 아니라 숙소의 실제 진입구나 합의한 도로 지점이어야 합니다." },
    { question: "가는 길에 호도협에 들를 수 있나요?", answer: "가능하지만 즉흥적인 길가 하차가 아니라 허가된 도로 이동으로 따로 설계해야 합니다. 날씨와 일광을 확인하고 마지막 픽업 지점을 지정하며, 계획한 정차에 쓸 낮 시간을 남겨 두세요. 열차와 비교하면 산악 도로 날씨와 교통, 기사 품질, 오래 앉아 있기가 주요 부담입니다." },
    { question: "도착한 뒤 몸이 안 좋으면 어떻게 하나요?", answer: "관광을 멈추고 진료를 받으세요. 심한 신호가 보이면 응급 서비스를 이용합니다. 심한 호흡 곤란과 심한 두통, 의식 변화는 즉시 진료가 필요하며 여행 글로 자가 진단하지 마세요. 의료진이 하산을 권하거나 일행이 안전하게 계속 갈 수 없다면, 선결제 일정보다 낮은 곳으로 경로를 바꿉니다." },
  ] },
