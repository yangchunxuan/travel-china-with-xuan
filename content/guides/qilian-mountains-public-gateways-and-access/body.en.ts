...
const body: StructuredPageBody = {schemaVersion: "1.0.0", blocks: [
  ...
  {id: "ledger-heading", type: "heading", level: 2, text: "Use a public-boundary ledger before choosing a base"},
  {id: "gateway-ledger", type: "table", caption: "What each Qilian access label proves—and does not prove", columns: ["Place or label", "What official evidence supports", "What you must not infer"], rows: [
    ...
    ["Hongwansi town, Sunan", "Zhangye forestry authorities identify the Qilian Mountains National Park Natural Museum in Hongwansi as an operating public-education asset.", "A museum visit does not authorise a self-drive route into the candidate area's core or prove every nearby road is open."],
    ...
    ["Babao town / Qilian County side", "The 2019 pilot plan listed Babao and nearby Qinghai communities as proposed entrance-service locations; newer sources confirm the Qinghai candidate section remains under conservation management.", "The old plan is not current operating proof. Verify a named public venue or licensed local activity for your date."],
  ...
  {id: "bases-heading", type: "heading", level: 2, text: "Choose the base by the public task you can actually verify"},
  {id: "base-choice", type: "table", caption: "Practical base choices for different Qilian trips", columns: ["Base", "Choose it when", "Build the day around"], rows: [
    ["Zhangye + Sunan / Hongwansi", "You want the strongest documented public interpretation and a road-based Zhangye-side trip.", "The Natural Museum, a separately confirmed Sunan scenic site and a return to town. Ask the venue for current hours and road conditions."],
    ["Zhangye + Mati township", "You want an established cultural attraction and an entrance-community context rather than a claim of wilderness access.", "The official Mati Temple scenic-area workflow, community services and a conservative return. Confirm ticketing, any internal transport and weather restrictions with the operator."],
    ["Qilian County / Babao", "Your wider Qinghai route already reaches Haibei and you can verify one named public attraction or registered local programme.", "That named product only. Do not cross from a county-base decision into unsupervised protected-area driving."],
    ["No overnight mountain base", "Only vague social-media coordinates are available, roads are affected, or the operator cannot identify the managing authority.", "Use a public museum or city-based nature-education day and postpone the mountain-edge component."]
  ]},
  {id: "workflow-heading", type: "heading", level: 2, text: "A lawful booking workflow"},
  {id: "lawful-workflow", type: "list", items: [
    "Write the exact destination in Chinese, including county, township and operator; ‘Qilian Mountains’ is not enough for a driver.",
    ...
    "For a driver, put the legal turnaround point, waiting time, mobile-signal failure plan and town return in writing.",
    ...
    "Recheck again the evening before departure; high-elevation road and fire-control decisions can change after an itinerary is booked."
  ]},
  ...
  {id: "traveller-scenarios", type: "table", caption: "Scenario decisions", columns: ["Traveller", "Better decision", "Reason"], rows: [
    ["Rail traveller with one spare day in Zhangye", "Use Zhangye as the hotel base; pair the Hongwansi museum with one separately verified public Sunan stop, or choose Mati township as a different day.", "It preserves a reliable city return and avoids selling the entire mountain range as a day-trip attraction."],
  ...
  {id: "conditions", type: "paragraph", text: "Long-term pattern: the Qilian range has large elevation changes, exposed roads and sharp local weather differences. That supports generous buffers, layered clothing and conservative daylight travel; it does not support a weather guarantee for the whole range. The 2026 fact is the candidate-area status. The Homeground judgment is to plan from the outside in—base, named operator, signed visitor boundary—rather than from an attractive coordinate outward."},
  {id: "final-check", type: "list", items: [
    "Current official status checked again before publication and departure.",
    "Exact county, township, venue and operator saved in Chinese.",
    "Public opening evidence is current, not only a pilot plan or promotion.",
    "Passport, ticket or registration method confirmed with the operator.",
  ...
  {id: "consultation", type: "callout", title: "Need a Qilian-side plan checked?", body: "A Homeground travel consultant can check the named gateway, operator, road chain and fallback for your dates. Share the exact towns, mobility needs and any activity you have been promised; we will not turn an unverified protected-area pin into a route.", tone: "neutral"},
