...
const body = {
  ...
  blocks: [
    ...
    },
    {
      ...
      title: "第一次去北京的实用默认答案",
      tone: "decision",
      body: "先按四到五个住宿夜规划，以王府井或东单一带作为稳定基地，给天安门—故宫一个完整日，给长城一个独立日，再从南部任务与西北任务中选一个。付款订酒店前先确认实际抵达机场和离京车站；在填入可移动的街区时间前，先锁定实名预约项目。若必须缩短，应整项删掉一个任务，而不是把长城和皇城中心压进同一天。",
    },
    {
      id: "nights-heading",
      type: "heading",
    ...
    },
    {
      ...
      type: "heading",
      level: 2,
      text: "按依赖顺序预订北京，不要按浏览顺序下单",
    },
    {
      id: "booking-order-table",
      type: "table",
      caption: "避免一个缺失确认拖垮全城的预订顺序",
      columns: ["顺序", "先决定什么", "为什么排在这里", "什么仍可保持弹性"],
      rows: [
        ["1", "国际抵达航班和已确认的下一程火车或航班", "它们决定机场、航站楼、准确车站和真正可用的住宿夜", "在实时抵达时刻明确前，机场接驳方式仍可比较"],
        ["2", "故宫和其他非去不可的实名场馆", "售罄可能迫使中轴日换日期；天安门广场仍须单独核验通行", "景山、北海、胡同与公园时段"],
        ["3", "长城段落与完整往返方式", "段落、天气暴露和返程链会占据独立一天", "哪一个城区日可以作为天气备用"],
        ["4", "住宿区域与具体物业", "此时门户组合、首个固定景点和行李路线已说明基地要承担什么", "具体餐厅与晚间安排"],
        ["5", "天坛、颐和园、博物馆与街区顺序", "稀缺依赖明确后，再用这些任务填完整条路线", "用于吸收天气、闭馆或疲劳的可移动日"],
      ],
    },
    {
      id: "stay-heading",
    ...
    },
    {
      ...
      type: "callout",
      title: "为什么有些北京资料仍写“七大火车站”？",
      tone: "neutral",
      body: "2025 年底以前发布的官方资料列出七座主要铁路站。北京通州站于 2025 年 12 月 30 日开通客运，北京铁路枢纽由此形成现行“八站”格局。这并不表示第一次来北京就应默认选择通州站：它位于城市副中心，主要服务部分东向城际线路。最终仍以车票和 12306 当前查询结果为准。",
    },
    {
      id: "gateways-2",
      type: "paragraph",
    ...
    },
    {
      ...
      type: "table",
      caption: "北京首日和末日能诚实完成什么",
      columns: ["交通形态", "可执行的城市安排", "不要叠加"],
      rows: [
        ["国际航班午后早段前抵达", "完成入境、取行李、接驳和入住；有体力再在酒店附近轻量活动", "实名分时景点或跨城晚间承诺"],
        ["国际航班晚间抵达", "抵达已订酒店、附近用餐，并准备次日证件", "演出、远端胡同预约，或没有恢复时间的长城早出发"],
        ["城际铁路抵达", "按准确车站前往酒店，只在住宿周边景点簇活动", "必须依赖火车与接驳完全准点的重大景点"],
        ["铁路或航班离京日", "把退房、前往车站或航站楼、安检与登乘视为主任务", "不能随时放弃的最后一个大景点"],
      ],
    },
    {
      id: "transport-links",
    ...
    },
    {
      ...
      columns: ["空间任务", "自然顺序", "当天前先锁定", "诚实的时间形态", "不应硬接什么"],
      rows: [
        [
          "天安门—故宫—景山",
          "从南向北的礼制与视觉序列",
          "故宫预约；如果明确参观广场，另行核验天安门通行",
          "一个扎实的城市中心日",
          "长城、赶飞机或远处固定时段博物馆",
        ],
        [
    ...
    },
    {
      id: "failure-table",
      type: "table",
      caption: "北京备用方案应保护地理关系与关键依赖",
      columns: ["故障", "先保护", "移动或替换"],
      rows: [
        ["故宫售罄或预约状态未确认", "长城日和下一程交通", "把故宫移到北京其他有票日期；否则使用连贯的中心公园—胡同日，不要押注所谓现场例外"],
        ["长城天气不佳或临时管控", "所选段落当前官方安全指引", "先使用可移动城区日；只有在开放、交通和后续天气都确认后，才调整长城日期"],
        ["中国国家博物馆预约失败", "中轴日真实的步行与安检负荷", "使用另一座已确认博物馆或街区时段；不要把地理相邻误当成可以入馆"],
        ["抵达延误或全组疲劳", "次日固定预约及证件原件", "删掉抵达夜景点，只保留用餐和酒店附近短走"],
        ["车站、航站楼或航班变化", "已出票交通及最低转场链", "先删末日游览，再考虑换酒店或购买脆弱的同日衔接"],
      ],
    },
    {
    ...
    },
    {
      id: "faq-9",
      type: "paragraph",
      text: "先确认航班和准确的下一程车站，再锁定故宫与其他非去不可的实名项目，然后确定长城段落与交通，最后才选择具体酒店并填入可移动景点簇。这样可以避免便宜酒店或随手列出的景点清单，反过来控制旅行中最稀缺的部分。",
    },
    {
      id: "cta-heading",
      type: "heading",
