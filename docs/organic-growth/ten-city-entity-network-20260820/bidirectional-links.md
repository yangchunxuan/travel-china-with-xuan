...
| Hangzhou | 7 | 0 | Prepare seven links, but activate atomically with formal release |
| Zhangjiajie | 10 | 0 | Prepare ten links, but activate atomically with formal release |
...
- Release gap: no stay specialist. Do not invent one as a link target.
...
- Release boundary: activate all return links only when central confirms the Hub is formally public.
...
1. Confirm lifecycle is formally `published`.
...
6. Re-run the five-owner gate using current-main content, not PR-only assumptions.
