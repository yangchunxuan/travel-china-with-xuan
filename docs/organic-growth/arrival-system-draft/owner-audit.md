...
PR integration checked: [PR #74](https://github.com/yangchunxuan/travel-china-with-xuan/pull/74), **MERGED** at `2026-08-20T15:17:58Z` (`2026-08-20 23:17:58` Asia/Shanghai), final head `b66fc6c`, merge commit `ef1898745a3c7a6e7cd308aa341c352f24fe9d01`
...
- Code integration is not the same state as publication. The release ledger/central instruction supports **29 existing published/live owners plus one code-integrated but not-live owner**, `china-online-arrival-card`. Do not count the latter as published until central performs explicit release and live verification.
- There is **one noindex canonical shell**, `/china-visa-free-uk-canada/`; it is not an owner and must never be revived.
- PR #74 integrated the complete `china-online-arrival-card` source package and asset into main, but the merged Search Map still records `approved / draft-submitted / not-published`, `liveUrls: []`, and dynamic status `approved-owner-not-published`. Central explicitly confirms it is **not live**. Its `datePublished` field and main-branch presence do not override that release state.
- PR #74 also integrated **in-place updates**, not new identities, for `do-singaporeans-need-visa-china` and `foreigners-china-hotel`.
- The Search Map merged by PR #74 records `planning-20260820-first-24-hours` as `deferred / not-started / not-published`. The new central assignment supersedes the no-writing instruction only far enough to create a reviewable draft package. It does **not** authorize a runtime route, indexation, merge or publication.
...
| `china-online-arrival-card` — `content/guides/china-online-arrival-card/*` | **Code-integrated in `origin/main@ef189874` via merged PR #74, but not live/published.** The merged Search Map remains `approved / draft-submitted / not-published`, `liveUrls: []`, status `approved-owner-not-published`; central independently confirms no online release. | **Canonical release candidate, not a live link target.** The Hub draft may reserve the task/slug and mark it “not live”; it must not claim or link as published. Before release, reopen the NIA form/host, fee, current eight-category interface versus seven-category 2025 notice, and port fallback. |
| `planning-20260820-first-24-hours` | Merged PR #74 Search Map: collection, `deferred`, `not-started`, `not-published`; explicitly route-only | **Current central assignment permits a documentation draft only.** No `content/guides` folder, app route, registry entry, sitemap URL or indexable page until central review. |
...
## PR #74 audit and release hazards

### Online arrival card: code merged, central release still not published
...
- The Search Map version merged in that PR is a pre-merge release-ledger snapshot: its top-level `mergeAuthorized: false` is historical, not the current Git state. Its item-level release record remains operative for publication status: `essentials-20260820-china-online-arrival-card` is `approved / draft-submitted / not-published`, with `liveUrls: []` and dynamic status `approved-owner-not-published`.
- Central explicitly confirms that no online release has occurred. The main-branch `metadata.json` value `datePublished: 2026-08-20` therefore must not be reported as a live publication date.
...
Main-branch presence and build eligibility are code-integration evidence, not public-release evidence. The generic guide adapter can make a generated node publishable/indexable, which raises a release hazard rather than proving a live page. Central must reopen the NIA host/form, deliberately approve deployment/indexation, verify the production URL in every locale, then update the Search Map from `not-published` to `published` with verified live URLs. Until all of those steps occur, the Hub may reserve the canonical slug but must label it **not live** and must not link to it as an available guide.
...
| P0 | `china-online-arrival-card` | Keep **code-integrated / not-live** release status; reopen the official host, form, free-service language, current eight-category interface versus seven-category 2025 notice, port/paper fallback and fraud warning before release. | Broken host, changed channel/exemption/fallback, unresolved seven/eight mismatch or unverified production URL blocks release and leaves the Hub label “canonical owner not live”. |
...
6. Do not create arrival-card pages by port, airline, language, device, exemption or “paid fast track”. `china-online-arrival-card` is the one code-integrated canonical release candidate, but it is not live; it must remain vendor-neutral and free-government-channel-first.
...
| Online arrival card fails | Stay on the official NIA channel; preserve accurate source records; use the current official port/device/paper fallback rather than a paid search result. | Code-integrated `china-online-arrival-card` release candidate; label not live until central release and URL verification |
...
Read-only commands used from `C:/Users/User/Documents/homeground-worker-6-china-arrival-system-20260820`:
...
- Selected generated arrival/practical owners in this audit: **23/23 represented in main code**, including the not-live arrival-card release candidate.
...
- Publication/release ledger count: **29 existing published/live owners + 1 code-integrated but not-live owner** (`china-online-arrival-card`).

## Central decision summary

- **Canonical/link**: link the 29 existing published/live owners. Reserve `china-online-arrival-card` as the sole canonical identity for its task, but do not expose it as a live link until central release and production verification.
...
- **Unpublished**: `china-online-arrival-card` code is in main but its central/Search Map release state is still not-published; the First-24-Hours Hub remains a review draft under the current assignment, not a public route.
- **Held**: entry eligibility checker.
- **Retired as an owner**: combined UK/Canada shell.
- **Highest cannibalization controls**: one system entry router; existing nationality owners only; one 240-hour owner; one code-integrated/not-live arrival-card owner; one eSIM owner; one payment-stack owner; one national hotel/registration owner; one Customs-channel owner plus one food/biosecurity owner; one route-only First-24-Hours Hub.
