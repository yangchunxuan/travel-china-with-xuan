...
1. [GitHub PR #74 — Complete SEO gap batch and synchronize Search Map](https://github.com/yangchunxuan/travel-china-with-xuan/pull/74)
   — checked again after the latest fetch on 2026-08-20. Merged into `main` at
   `ef1898745a3c7a6e7cd308aa341c352f24fe9d01`. Its files now exist on the
   baseline, but the merged Search Map still records `china-online-arrival-card`
   as `not-published` with no live URLs; central independently confirms it is
   not online.
2. `origin/main@ef1898745a3c7a6e7cd308aa341c352f24fe9d01` — fetched
   2026-08-20 and used for every repository-state and inventory decision. File
   presence is kept separate from live-publication state.
