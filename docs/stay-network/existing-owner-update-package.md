...
### Seven published destination Hubs

Beijing, Shanghai, Xi'an, Chengdu and Guangzhou receive aligned EN/ZH/KO
changes. Hangzhou and Zhangjiajie receive the same decision handoffs through
their shared trilingual bodies. All seven are published Hubs:
...
Only `dateModified` changes to 2026-08-21. Existing `sourceReviewedDate` values
...
| Zhangjiajie owner + published Hub | implemented update | no second city-versus-Wulingyuan comparison |
| Hangzhou published Hub | implemented stay-network/FAQ update | no `hg-topic-0498` broad owner |
...
The seven changed Hubs and the selected city owners already use real repository
...
1. Retain PR #74's published Hangzhou/Zhangjiajie routes and related pages
   during manual integration.
2. Follow `employee-2-manual-merge-note.md`; do not replace the seven-Hub
   registry with an older five-published/two-candidate lifecycle snapshot.
...
5. Do not edit the four Search Map main-ledger files in this branch; Employee 7
   owns that synchronization. This package corrects only stay-owned audit docs,
   the Hub draft README/static test and public content state.
