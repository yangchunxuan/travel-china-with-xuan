...
Review baseline: 2026-08-21 on `origin/main` `ef189874`, where PR #74 is
merged and its Hangzhou/Zhangjiajie routes are published.
...
If a new cleared real asset is unavailable, write a precise shot request and
retain any truthful existing image. A missing enhancement does not block an
otherwise reviewable text update to an already published page. Never fill the
slot with a deceptive synthetic scene.
...
| City | Current documentary coverage | Remaining decision-network image gap |
| --- | --- | --- |
...
| Zhangjiajie | Published Hub hero is a real repository landscape; city-versus-Wulingyuan owner has a real hero | Obtain distinct city gateway, Wulingyuan street/entrance and luggage-transfer scenes; a mountain panorama does not prove either hotel base. |
| Hangzhou | Published Hub hero is a real repository West Lake image | Obtain Hubin/east-shore, west-side hill-road and Hangzhou East lodging-buffer scenes; the lake hero does not prove property access. |
...
### Page-level rights and scene gaps

| Page or group | Current evidence | Gap to close without blocking this text update |
| --- | --- | --- |
| `foreigners-china-hotel` | Commons CC BY-SA 4.0 record includes creator, source, place/date and derivative hash | No identified rights gap; keep the image review record at 2026-08-13 because the 2026-08-20 policy recheck did not relicense the image. |
| `china-accessible-hotel-room-verification` | Commons CC BY-SA 4.0 source and derivative hash are recorded | No identified rights gap; remove any contradictory “assets needed” note that says no hero exists, and keep the image review date separate from the factual review. |
| `china-last-night-before-international-flight` | Real Homeground library image and derivative hash exist | The library-use direction is not independent proof of the original photographer's commercial rights. Recognizable travellers also require a documented release or a replacement crop/image. Retain the text update while central closes the creator/licence and portrait chain. |
| Beijing, Shanghai, Xi'an and Chengdu Hub library images | Real repository files and project-owner publication direction | Add original creator, capture metadata, written commercial/translation/crop scope and any needed releases. Do not broaden captions beyond the recorded scene. |
| Guangzhou Hub and Shanghai/Xi'an/Chongqing/Shenzhen owner Commons images | Named creator, source, compatible licence and derivative record | Preserve attribution, licence/share-alike duties and edit notes; acquire missing lodging-last-metre scenes separately. |
| Hangzhou and Zhangjiajie Hub library images | Real project-owner-supplied production files | Photographer/capture metadata and a fuller licence chain remain incomplete. The images orient the destination only; they prove no hotel access, inventory or room feature. |
| `zhangjiajie-city-or-wulingyuan-hotel-base` | Real Wulingyuan-area local-library hero with project permission and hash | Creator, capture date, people/property releases and scope need stronger records. Reconcile its image plan, which still contains both “hero selected” and “no image committed” language. The current frame does not prove the city base. |
| `beijing-courtyard-hotel-or-modern-hotel`, `commercial-aparthotel-or-residential-rental-china`, `minsu-homestay-or-hotel-china`, `serviced-apartment-or-hotel-china` | Diagram-only hero treatment | Commission or license a real, accurately located accommodation/street scene. Keep diagrams clearly illustrative until then; never substitute an AI hotel, room or street. |
