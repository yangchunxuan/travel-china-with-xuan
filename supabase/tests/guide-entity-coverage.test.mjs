...
test("entity coverage includes the complete generated plus legacy runtime ledger", async () => {
  ...
  assert.deepEqual(report.scope, {
    runtimeGuideCount: 240,
    independentGuideCount: 221,
    legacyGuideCount: 19,
  });
  assert.equal(report.guideCount, 240);
  assert.equal(report.guideWithUnmappedTokenCount, 115);
  assert.equal(report.countryFallbackGuideCount, 50);
  assert.equal(report.unmappedTokenCount, 169);
