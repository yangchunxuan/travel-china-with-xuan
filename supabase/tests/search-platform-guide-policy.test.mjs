...
function parseLegacyFreshnessInputs(source) {
  ...
  return matches.map((match, index) => {
    ...
    return {
      id: match[1],
      pillar,
      topics: [...topicsSource.matchAll(/"([^"]+)"/gu)].map(
        (topic) => topic[1],
      ),
    };
  });
...
test("all six Guangzhou hub support guides resolve the Guangzhou entity", async () => {
  ...
  for (const id of ids) {
    ...
    assert.ok(
      resolveGuideEntities(metadata.destinations).entityIds.includes(
        "city-guangzhou",
      ),
      id,
    );
...
test("freshness registry exhaustively covers the runtime pillar vocabulary", async () => {
  ...
  assert.equal(runtime.size, 195);
  const runtimePillars = [
    ...new Set([...runtime.values()].map((guide) => guide.pillar)),
  ].sort();
...
test("explicit high-risk minima cover safety, accessibility, airline and dynamic visitor guides", async () => {
  ...
  assert.equal(
    guideUpdatePolicy(runtime.get("china-domestic-flight-schedule-change"))
      .volatility,
    "critical",
  );
});
