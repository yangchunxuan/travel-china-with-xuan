...
-- Carries only a server-verified published-tour identity through the existing
-- email-only homepage inquiry. Free text, itinerary facts and query-provided
-- product names remain forbidden.
...
begin
  ...
  expected_attribution := case
    ...
    when expected_product_name is not null then jsonb_build_object(
      'productInterest',
      jsonb_build_object(
        'slug', product_slug,
  ...
  homepage_answers := jsonb_build_object(
    'informationStatus',
    'not_provided'
  ) || case
    when expected_product_name is null then '{}'::jsonb
...
comment on function public.create_homeground_homepage_email_v1(
  ...
  'Creates or replays an email-only homepage contact request and may retain one strictly allowlisted published-tour identity. No free-text itinerary facts are accepted.';
